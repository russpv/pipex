...
static int _redirect(int *to, char *topath, int from)
{
	...
	{
		if (from == STDIN_FILENO && access(topath, R_OK) == -1) // bad cmd last round
		{
			ft_printf("warning: An error occurred while redirecting file '%s'", topath); fflush(stdout);
	...
	while (++i < st.cmd_count) {
		...
		{ 
			...
			if (st.cmdpaths[i] == NULL)
				...
				{
					ft_printf("err:%d", errno); fflush(stdout);
					sleep(1);
